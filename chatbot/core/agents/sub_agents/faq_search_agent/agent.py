...
# Get the FAQ server URL from environment variable or use localhost as default
FAQ_SERVER_URL = os.getenv("FAQ_SERVER_URL", "http://localhost:8001/sse")
...
def create_agent(query_index: int, query: Optional[str] = None) -> LlmAgent:
    ...
    agent = LlmAgent(
        ...
        tools=[MCPToolset(connection_params=SseConnectionParams(url=FAQ_SERVER_URL))],
