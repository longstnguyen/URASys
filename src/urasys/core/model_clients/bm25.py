...
from urasys.utils.embeddings import SparseEmbedding
...
class BM25Client:
    ...
    def csr_to_dict(csr: csr_array | csr_matrix) -> Dict[int, float]:
            ...
            mat = csr.tocsr()
            if mat.shape[0] != 1:
                raise ValueError("Input must have exactly 1 row")
    ...
    def encode_text(self, data: List[str]) -> List[SparseEmbedding]:
        ...
        # Return each row as a (1, vocab_size) csr_array for Milvus compatibility
