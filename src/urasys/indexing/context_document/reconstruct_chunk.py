...
from urasys.indexing.context_document.base_class import ExtractedContext, ReconstructedChunk
from urasys.prompts.indexing.generate_title import GENERATE_TITLE_QUICK_DESCRIPTION_PROMPT_TEMPLATE
from urasys.prompts.indexing.rewrite_chunk import REWRITE_TEXT_CHUNK_PROMPT_TEMPLATE
...
class ChunkReconstructor:
    ...
    def reconstruct_chunks(self, chunks: List[str], context: ExtractedContext) -> List[ReconstructedChunk]:
        ...
        for chunk in chunks:
            # Step 1: Rewrite the chunk with document-level context
            rewritten_chunk = self.rewrite_chunk(chunk, context)

            # Step 2: Generate a concise title for the rewritten chunk
            title = self.generate_title_quick_description(rewritten_chunk)

            # Step 3: Combine title and chunk → x_i = Concat(t_i, c_i)
